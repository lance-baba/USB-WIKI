...
| Attribution Precision | 54.5% | **100.0%** |
| Attribution Recall | 100.0% | **100.0%** |
| False Association Rate | 83.3% | **0.0%** |

（正例 12 / 负例 9 / 无答案 3）

> 主口径只统计 **Router 交给 Guard 的 24 题**（causal/event/responsibility）；另有 9 题按设计走 PASS_THROUGH（property/confusion/impact），不进入 Guard 指标。
## 逐题判定（guard）
...
| ad_i01 | impact | yes | `YES` | PASS_THROUGH（不进入归因守卫） |
| ad_i02 | impact | yes | `YES` | PASS_THROUGH（不进入归因守卫） |
| ad_i03 | impact | yes | `YES` | PASS_THROUGH（不进入归因守卫） |
| ad_p01 | property | yes | `YES` | PASS_THROUGH（不进入归因守卫） |
| ad_p02 | property | yes | `YES` | PASS_THROUGH（不进入归因守卫） |
| ad_p03 | property | yes | `YES` | PASS_THROUGH（不进入归因守卫） |
| ad_p06 | property | no | `YES` | PASS_THROUGH（不进入归因守卫） |
...
| ad_x02 | confusion | no | `YES` | PASS_THROUGH（不进入归因守卫） |
| ad_x03 | confusion | no | `YES` | PASS_THROUGH（不进入归因守卫） |
...
- 关系型查询数：25 / 72
- ⚠ **guard 在 DEV 72 上误拦了 2 道本可回答的普通题** → 说明 guard 不能无差别地套在所有关系型查询上
- 端到端耗时（检索+guard）：3.32 ms/query
- **guard 自身耗时：2.44 ms/query**（目标 < 20ms）

...
> **已实测**：`runtime/python-3.11-embed/python.exe tests/test_suite.py` → **172/172 通过**
...
│   └── cache.db                # 衍生索引（可全量重建）
└── tests/
    ├── test_suite.py           # 172 项自动化测试（数据根目录隔离到临时目录）
...
```bash
python tests/test_suite.py     # 172 项：切片/检索/网关/星图/破坏性/三项隐蔽细节/数据安全
